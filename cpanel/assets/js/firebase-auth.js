(function () {
  ...
  function currentProfile() {
    const auth = initFirebase();
    if (!auth || !auth.currentUser) return { idToken: null, uid: null };
    const user = auth.currentUser;
    return { user, idToken: user.getIdToken(), uid: user.uid };
  }
  ...
  window.XD_SMS.firebaseRegister = async function (name, email, password) {
    ...
    const res = await window.XD_SMS.api("/api/auth/register", {
      method: "POST",
      body: JSON.stringify({ name, email, idToken, uid }),
